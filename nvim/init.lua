...
vim.o.winborder = 'rounded' -- add window boarder to highlight floated windows
...
-- Offset for smoother scrolling - if the value is bigger than half the screen then I believe it should always scroll
...
vim.opt.spell = true -- spelling - treesitter by default disables spellchecks in code, allowing it in MD files and comments.
vim.opt.spelllang = { 'en', 'pl' } -- ...in the kings language
...
vim.keymap.set('n', '<Leader>vd', vim.diagnostic.open_float, { desc = 'Open diagnostic for current line' })
...
-- Copying the current file's relative path to the clipboard
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<C-p>', builtin.builtin, { desc = 'List [P]ickers' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- map('<C-F>', vim.lsp.buf.format, '[F]ormat current buffer')
  ...
  -- Image pasting, displaying, MD features.
  {
    '3rd/image.nvim',
  },
  {
